...
import { Key, Lock, AlertTriangle, PlayCircle, Zap, Globe, Copy, ExternalLink, X } from 'lucide-react';
...
export const ConfigModal: React.FC<ConfigModalProps> = ({ onSave, onClose, isProcessingAuth }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
      <div className="bg-gray-900 border border-gray-700 rounded-xl shadow-2xl max-w-md w-full overflow-hidden flex flex-col max-h-[90vh]">
        ...
        {/* Header */}
        <div className="bg-gradient-to-r from-blue-900 to-indigo-900 p-6 border-b border-gray-700 flex justify-between items-start">
          <div>
            ...
            <p className="text-blue-200 text-sm mt-2">
              Securely connect to Fyers V3 API.
            </p>
          </div>
          <button 
            onClick={onClose}
            className="text-blue-200 hover:text-white p-1 hover:bg-white/10 rounded transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
